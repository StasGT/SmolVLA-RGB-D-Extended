    ...
    def add_frame(self, frame: dict) -> None:
        ...
        for key in frame:
            ...
            if self._meta.features[key]["dtype"] == "video" and self._streaming_encoder is not None:
                ...
                compress_level = 1
                self._save_image(frame[key], img_path, compress_level)
                self._meta.features[key] = {'dtype': 'video', 'shape': (480, 640, 3), 'names': ['height', 'width', 'channels']}
                self.episode_buffer[key] = [str(img_path)]
                #print("---------------------", self.episode_buffer.keys())
                #print(key)
                #print(self.episode_buffer[key])

            else:
                self.episode_buffer[key].append(frame[key])

        #print(self._meta.features)
