...
class SOFollower(Robot):
    ...
    def get_observation(self) -> RobotObservation:
        ...
        for cam_key, cam in self.cameras.items():
            ...
            if isinstance(cam, RealSenseCamera) and self.config.cameras[cam_key].use_depth == True:
                clipped = np.clip(cam.read_depth(), 300, 600) #clip length 300 to 600 millimeters
                depth_image = ((clipped - 300) * 0.85).astype(np.uint8) # remap cliped to uint8
                #obs_dict['depth'] = np.expand_dims(depth_image, axis=-1)
                obs_dict['depth'] = np.repeat(np.expand_dims(depth_image, axis=-1), 3, axis=2)
